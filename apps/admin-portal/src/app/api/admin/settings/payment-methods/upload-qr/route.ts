...
export async function POST(request: NextRequest) {
  try {
    ...
    const uniqueFileName = `payment-qrs/${Date.now()}-${Math.random().toString(36).substring(2, 9)}.${extension}`;
    ...
    if (!uploadRes.ok) {
      ...
      return NextResponse.json(
        { error: errData.message || 'Failed to upload QR image to storage' },
    ...
    return NextResponse.json(
      { error: error?.message || 'Error processing QR image upload' },
      { status: 500 }
    );
  }
}
